...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| hatexplain/uncalibrated | 5000 | 0.6974 | 0.0259 | 0.0249 | 0.4101 | 63.2 | 65.7 | 92 |
| hatexplain/calibrated | 5000 | 0.6974 | 0.0259 | 0.0249 | 0.4101 | 63.3 | 65.5 | 92 |

## Release gates

- PASS sample_size: 5000.0000 (limit 5000.0000) -- below this, ECE is dominated by estimator noise
- PASS gate_is_testable: 0.0200 (limit 0.0250) -- a perfectly calibrated model scores ECE 0.0142 on this run, p95 0.0200
- PASS accuracy_over_baseline: 0.2896 (limit 0.0500) -- model 0.6974 vs marginal predictor 0.4078; calibration cannot reject a model that ignores the state
- PASS (advisory) worst_question_over_baseline: 0.2896 (limit 0.0500) -- worst is 'label' at 0.6974 vs its own marginal 0.4078; the pooled gate hides this
- PASS workhorse_ece: 0.0259 (limit 0.0500)
- PASS workhorse_adaptive_ece: 0.0249 (limit 0.0500)
- PASS (advisory) worst_primitive_workhorse_ece: 0.0259 (limit 0.0500) -- worst is 'choice' at ECE 0.0259 (overconfidence +0.012); pooled ECE cancels heads that err in opposite directions
...
| --- | ---: | ---: | ---: | ---: |
| `label` | 5,000 | 0.6974 | 0.4078 | +0.2896 |

## Per-domain calibration

| Domain | n | Accuracy | Mean confidence | Overconfidence | ECE | Adaptive ECE |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| hatexplain | 5000 | 0.697 | 0.710 | +0.012 | 0.0259 | 0.0249 |

## Per-primitive calibration

| Primitive | n | Accuracy | Mean confidence | Overconfidence | ECE | Adaptive ECE |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| choice | 5000 | 0.697 | 0.710 | +0.012 | 0.0259 | 0.0249 |

## Is this number evidence?

On these 5000 predictions a perfectly calibrated model scores a mean ECE of 0.0142 (95th percentile 0.0200), simulated over 200 resamples. The measured ECE is 0.0259.
...
```
  bin            n    conf     acc     gap
  ----------------------------------------------------------------------
  [0.33,0.40)   104  0.379  0.394  -0.016  ###############|........................
  [0.40,0.47)   407  0.436  0.484  -0.048  #################|#.....................
  [0.47,0.53)   489  0.500  0.448  +0.052  ##################..|...................
  [0.53,0.60)   499  0.567  0.539  +0.028  ######################.|................
  [0.60,0.67)   521  0.634  0.643  -0.009  #########################|..............
  [0.67,0.73)   572  0.699  0.713  -0.014  ############################|...........
  [0.73,0.80)   584  0.767  0.738  +0.029  ##############################.|........
  [0.80,0.87)   705  0.834  0.826  +0.009  #################################|......
  [0.87,0.93)   609  0.897  0.867  +0.030  ###################################.|...
  [0.93,1.00)   510  0.964  0.935  +0.029  #####################################..|
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| model | `span_head` | 200 | 0.6994 | 0.7851 | 0.7961 | 0.5685 | 0.340 | 0.356 |
| model, gradient x input | `gradient_x_input` | 200 | 0.2717 | 0.5227 | 0.2602 | 0.1919 | 0.128 | 0.356 |
| model, integrated gradients | `integrated_gradients` | 200 | 0.4180 | 0.6673 | 0.3979 | 0.3632 | 0.146 | 0.356 |
...
Integrated gradients' completeness on the first 200 of these cases (256 points, `u ** 3` spacing, float32 path): relative error of the summed attributions against the log-probability difference they must add up to, median 0.6487, p90 2.6397, max 17.3428, over the 198 whose difference is at least 0.01 nats (median difference 2.417). A large error means too few points or too little precision, and the `integrated gradients` row is then an artefact of the arithmetic, not the method.
