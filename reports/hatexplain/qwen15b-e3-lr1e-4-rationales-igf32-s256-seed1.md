...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| hatexplain/uncalibrated | 5000 | 0.6860 | 0.0201 | 0.0245 | 0.4197 | 65.7 | 73.3 | 92 |
| hatexplain/calibrated | 5000 | 0.6846 | 0.0279 | 0.0279 | 0.4229 | 65.0 | 99.5 | 92 |

## Release gates

- PASS sample_size: 5000.0000 (limit 5000.0000) -- below this, ECE is dominated by estimator noise
- PASS gate_is_testable: 0.0175 (limit 0.0250) -- a perfectly calibrated model scores ECE 0.0111 on this run, p95 0.0175
- PASS accuracy_over_baseline: 0.2758 (limit 0.0500) -- model 0.6846 vs marginal predictor 0.4088; calibration cannot reject a model that ignores the state
- PASS (advisory) worst_question_over_baseline: 0.2758 (limit 0.0500) -- worst is 'label' at 0.6846 vs its own marginal 0.4088; the pooled gate hides this
- PASS workhorse_ece: 0.0279 (limit 0.0500)
- PASS workhorse_adaptive_ece: 0.0279 (limit 0.0500)
- PASS (advisory) worst_primitive_workhorse_ece: 0.0279 (limit 0.0500) -- worst is 'choice' at ECE 0.0279 (overconfidence +0.028); pooled ECE cancels heads that err in opposite directions
...
| --- | ---: | ---: | ---: | ---: |
| `label` | 5,000 | 0.6846 | 0.4088 | +0.2758 |

## Per-domain calibration

| Domain | n | Accuracy | Mean confidence | Overconfidence | ECE | Adaptive ECE |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| hatexplain | 5000 | 0.685 | 0.712 | +0.028 | 0.0279 | 0.0279 |

## Per-primitive calibration

| Primitive | n | Accuracy | Mean confidence | Overconfidence | ECE | Adaptive ECE |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| choice | 5000 | 0.685 | 0.712 | +0.028 | 0.0279 | 0.0279 |

## Is this number evidence?

On these 5000 predictions a perfectly calibrated model scores a mean ECE of 0.0145 (95th percentile 0.0209), simulated over 200 resamples. The measured ECE is 0.0201.
...
```
  bin            n    conf     acc     gap
  ----------------------------------------------------------------------
  [0.33,0.40)    95  0.381  0.400  -0.019  ###############|........................
  [0.40,0.47)   431  0.436  0.413  +0.023  #################|......................
  [0.47,0.53)   608  0.500  0.480  +0.020  ###################.|...................
  [0.53,0.60)   582  0.567  0.558  +0.008  ######################.|................
  [0.60,0.67)   577  0.636  0.674  -0.039  #########################|#.............
  [0.67,0.73)   633  0.700  0.722  -0.022  ############################|...........
  [0.73,0.80)   604  0.765  0.781  -0.017  ###############################|........
  [0.80,0.87)   618  0.835  0.838  -0.003  #################################|......
  [0.87,0.93)   537  0.899  0.864  +0.035  ###################################.|...
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| model | `span_head` | 200 | 0.7293 | 0.8240 | 0.7743 | 0.6495 | 0.317 | 0.349 |
| model, gradient x input | `gradient_x_input` | 200 | 0.3145 | 0.5220 | 0.3265 | 0.2387 | 0.148 | 0.349 |
| model, integrated gradients | `integrated_gradients` | 200 | 0.4446 | 0.7327 | 0.4182 | 0.3937 | 0.133 | 0.349 |
...
Integrated gradients' completeness on the first 200 of these cases (256 points, `u ** 3` spacing, float32 path): relative error of the summed attributions against the log-probability difference they must add up to, median 0.7228, p90 2.5918, max 23.7352, over the 196 whose difference is at least 0.01 nats (median difference 1.219). A large error means too few points or too little precision, and the `integrated gradients` row is then an artefact of the arithmetic, not the method.
