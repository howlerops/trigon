...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| hatexplain/uncalibrated | 5000 | 0.5698 | 0.0264 | 0.0223 | 0.5335 | 68.0 | 73.1 | 92 |
| hatexplain/calibrated | 5000 | 0.5698 | 0.0264 | 0.0223 | 0.5335 | 67.3 | 72.7 | 92 |

## Release gates

- PASS sample_size: 5000.0000 (limit 5000.0000) -- below this, ECE is dominated by estimator noise
- PASS gate_is_testable: 0.0214 (limit 0.0250) -- a perfectly calibrated model scores ECE 0.0147 on this run, p95 0.0214
- PASS accuracy_over_baseline: 0.1610 (limit 0.0500) -- model 0.5698 vs marginal predictor 0.4088; calibration cannot reject a model that ignores the state
- PASS (advisory) worst_question_over_baseline: 0.1610 (limit 0.0500) -- worst is 'label' at 0.5698 vs its own marginal 0.4088; the pooled gate hides this
- PASS workhorse_ece: 0.0264 (limit 0.0500)
- PASS workhorse_adaptive_ece: 0.0223 (limit 0.0500)
- PASS (advisory) worst_primitive_workhorse_ece: 0.0264 (limit 0.0500) -- worst is 'choice' at ECE 0.0264 (overconfidence -0.014); pooled ECE cancels heads that err in opposite directions
...
| --- | ---: | ---: | ---: | ---: |
| `label` | 5,000 | 0.5698 | 0.4088 | +0.1610 |

## Per-domain calibration

| Domain | n | Accuracy | Mean confidence | Overconfidence | ECE | Adaptive ECE |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| hatexplain | 5000 | 0.570 | 0.556 | -0.014 | 0.0264 | 0.0223 |

## Per-primitive calibration

| Primitive | n | Accuracy | Mean confidence | Overconfidence | ECE | Adaptive ECE |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| choice | 5000 | 0.570 | 0.556 | -0.014 | 0.0264 | 0.0223 |

## Is this number evidence?

On these 5000 predictions a perfectly calibrated model scores a mean ECE of 0.0147 (95th percentile 0.0214), simulated over 200 resamples. The measured ECE is 0.0264.
...
```
  bin            n    conf     acc     gap
  ----------------------------------------------------------------------
  [0.33,0.40)   736  0.374  0.399  -0.025  ###############|........................
  [0.40,0.47)  1238  0.434  0.422  +0.011  #################|......................
  [0.47,0.53)   984  0.496  0.534  -0.038  ####################|...................
  [0.53,0.60)   497  0.563  0.588  -0.024  #######################|................
  [0.60,0.67)   303  0.632  0.700  -0.067  #########################|##............
  [0.67,0.73)   239  0.701  0.661  +0.040  ##########################..|...........
  [0.73,0.80)   320  0.768  0.803  -0.035  ###############################|........
  [0.80,0.87)   349  0.835  0.814  +0.021  #################################|......
  [0.87,0.93)   287  0.899  0.906  -0.007  ####################################|...
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| model | `gradient_x_input` | 2,956 | 0.1793 | 0.3871 | 0.1647 | 0.1430 | 0.101 | 0.337 |
| model, integrated gradients | `integrated_gradients` | 2,956 | 0.1934 | 0.3681 | 0.1822 | 0.1483 | 0.117 | 0.337 |
...
Integrated gradients' completeness on the first 200 of these cases (32 points, `u ** 3` spacing): relative error of the summed attributions against the log-probability difference they must add up to, median 8.9536, p90 110.7961, max 3015.0357, over the 200 whose difference is at least 0.01 nats (median difference 0.564). A large error means too few points, and the `integrated gradients` row is then a quadrature artefact, not the method.
