...
def main() -> int:
    ...
    for seed in known.seeds:
        ...
        if result.returncode not in (0, 1):
            print(f"seed {seed} failed to run ({result.returncode})", file=sys.stderr)
        ...
        rows.append(
            {
                ...
                "certified": all(g["passed"] for g in gates),
            }
        )
        if not known.keep_reports:
            for extra in out_dir.glob(f"{known.name}-seed{seed}*"):
                if extra.suffix in {".pt"}:
                    extra.unlink()

    print()
    print("| Seed | Final loss | Kept epoch | Lift over baseline | ECE | Certified |")
    print("| ---: | ---: | ---: | ---: | ---: | --- |")
    for row in rows:
